...
plot(diff(log(x[,2])), type="l",main="Daily Apple Stock Prices Log Returns 10/2014-10/2015",ylab="Log Returns",xlab="Day")

#Look for stable mean to make sure there is no trend in the mean
...
# Kwiatkowski-Phillips-Schmidt-Shin Test KPSS 
kpss.test(diff(xt)) # if greater than 0.05, than you have a first-order stationarity time series with 95% of confidence
...
grw1 <- rwf(log(xt),h=22) # Log
grw2 <- Arima(log(xt), order=c(0,1,0)) # Log
# Chart
...
# 4.11. Gardner's Additive Damped Trend ARIMA(1,1,2) without constant (1 autoregressive term, 1 order of integration or differentiation and 2 moving average terms)
...
lines(x)
# ARIMA(0,0,0)x(0,1,0)m --> 0 autoregressive terms, 0 order of integration/differentiation, 0 moving average terms, x (0,1,0) --> 0 seasonal autoregressive terms, 1 order of seasonal integration/differentiation, 0 seasonal moving average terms 
# ARIMA Coefficients
summary(srwd2)
...
# 4.17. Holt-Winters Additive Seasonality ARIMA (0,1,6)X(0,1,0)5 (m) without constant
...
Box.test(residuals(abest),lag=10,type="Ljung") # 10 lags is industry standard
# pvalue > .05 therefore no autocorrelation or serial correlation within forecasting errors/residuals with 95% confidence
# this course does not take into account Kurtosis, variance, asymetry, outside the scope
...
#s.e.  0.1180
#csign=.429661
#.42966<2.0322 not significant
#therefore the holt model or the random walk model w/o drift may be better
#forecasting fitting the data vs. complexity
##########